...
import com.ADCS441.cloudsim.utils.ConfigApplications
import com.ADCS441.tablebuilder.{CustomTableBuilder, DataCenterBrokerCustom}
...
class PaasSimulation extends LazyLogging {
  ...
  val dc4 = base.createDataCenter(config.HOSTS_NO,config.SERVICE_PROVIDER.getString("datacenter4"),config.DATACENTER_TYPE.getString("Simple"),config.FIRST_FIT,simulation,config.HOST_TYPE.getString("host2"))
  ...
  println(" ")
  println("Select a Datacenter to which the task should be assigned")
  println("Choose from Options - 1, 2, 3, 4")
  ...
  val broker =
    c match {
      ...
      case 4  => new DataCenterBrokerCustom (simulation, 3)
    }
  println("")
  ...
  val cloudletList = cloudlet.Cloudlets(config.NO_OF_CLOUDLETS,config.CLOUDLET_TYPE.getString("cloudlet"),config.CLOUDLETS_CPU_UTIL.getDouble("full"))
