...
import com.ADCS441.cloudsim.Base.{CloudletBase, DataCenterBase, NetworkEntities}
...
class SaasSimulation extends LazyLogging{
  ...
  network_entity.Network(nw_simulation_3,nw_datacenter.asInstanceOf[NetworkDatacenter],config.SERVICE_PROVIDER.getString("datacenter1"))
  val broker = new DatacenterBrokerSimple(nw_simulation_3)
  val cloudletList = new CloudletBase().ListNetworkCloudlets(nw_vmList)
  ...
  private def showSimulationResults(): Unit = {
    ...
    println(" ")
    println(getClass.getSimpleName + "Done")
  }
