package com.ADCS441.cloudsim.Base
...
class NetworkEntities extends DataCenterBase {
   ...
   * @param datacenter
   */
  def Network(simulation: CloudSim,datacenter: NetworkDatacenter,ServiceProvider:String):Unit = {
    val configDATACENTERS =
      ServiceProvider match {
        ...
        case "datacenter4" => configs.DATACENTERS4
    ...
    val edgeswitches = EdgeSwitches(simulation: CloudSim,datacenter: NetworkDatacenter,aggregateswitches,configDATACENTERS.getConfig(configs.EDGE_SWITCH),configDATACENTERS)
   ...
   */
  def EdgeSwitches(sim: CloudSim, datacenter: NetworkDatacenter, aggregate: List[Switch], config: Config,configDATACENTERS:Config): List[Switch] ={
