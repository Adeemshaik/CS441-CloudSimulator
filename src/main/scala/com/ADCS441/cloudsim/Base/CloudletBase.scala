...
*/
   ...
   */
  def Cloudlets(Cloudlet_No:Int,Cloudlet_type:String,PERCENT:Double) : List[Cloudlet] ={
    val configs =
      Cloudlet_type match {
  ...
  def CloudletInVM(VM_LIST:List[Vm], PERCENT:Double):util.ArrayList[Cloudlet] ={
  ...
  def CPUUtilization(PERCENT:Double): UtilizationModelDynamic ={
   ...
   def ListNetworkCloudlets(VM_LIST:List[Vm]):List[NetworkCloudlet] = {
    val networkCloudletList = (1 to config.saas_cloudlet.getInt("cloudlets_num")).map{
      x =>
        NetworkCloudlet(VM_LIST(x).asInstanceOf[NetworkVm])
    }.toList
   ...
   * @return
   */
   def NetworkCloudlet(vm: NetworkVm):NetworkCloudlet = {
